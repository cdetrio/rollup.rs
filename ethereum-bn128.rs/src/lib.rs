...
pub fn bn128_pairing(input: &[u8], output: &mut [u8; 32]) -> Result<(), Error> {
    use bn::{pairing_batch, pairing, AffineG1, AffineG2, Fq, Fq2, Group, Gt, G1, G2};
    ...
    let ret_val = if input.len() == 0 {
        ...
        for idx in 0..elements {
            ...
            let b = if b_a.is_zero() && b_b.is_zero() {
                ...
                //println!("creating G2 from affineG2 b_a: {:?}, b_b: {:?}", b_a, b_b);
                G2::from(
            ...
            let a = if a_x.is_zero() && a_y.is_zero() {
                ...
                //println!("creating G1 from affineG1 a_x: {:?}, a_y: {:?}", a_x, a_y);
                G1::from(
                    AffineG1::new(a_x, a_y)
                        .map_err(|_| Error::from("Invalid a argument - not on curve"))?,
                )
            };
            //println!("bn128_pairing G1: {:?}", a);
            vals.push((a, b));
        }
        ...
        let mul = vals
            ...
            .fold(Gt::one(), |s, (a, b)| {
                    let pairing_result = pairing(a, b);
                    println!("pairing_result: {:?}", pairing_result);
                    let s_times_pairing_result = s * pairing_result;
                    println!("s_times_pairing_result: {:?}", s_times_pairing_result);
                    s_times_pairing_result
                }
            );
        */
