...
// this NAF (from the python code) gets the same pairing result, but the G2Precomp coefficients are different
const ATE_LOOP_COUNT_NAF : [u8; 64] = [1, 0, 1, 0, 0, 3, 0, 1, 1, 0, 0, 0, 3, 0, 0, 1, 1, 0, 0, 3, 0, 0, 0, 0, 0, 1, 0, 0, 3, 0, 0, 1, 1, 1, 0, 0, 0, 0, 3, 0, 1, 0, 0, 3, 0, 1, 1, 0, 0, 1, 0, 0, 3, 1, 0, 0, 3, 0, 1, 0, 1, 0, 0, 0];
...
impl AffineG<G2Params> {
    ...
    pub fn precompute(&self) -> G2Precomp {
        ...
        let q_neg = self.neg();
        for i in ATE_LOOP_COUNT_NAF.iter() {
            // doubling_step_for_flipped_miller_loop() returns a EllCoeffs type, which has 6 coordinates (G2 point in Jacobian form)
            coeffs.push(r.doubling_step_for_flipped_miller_loop());

            if *i == 1 {
                // mixed_addition_step_for_flipped_miller_loop() also returns a EllCoeffs type.
        ...
        //for co_i in coeffs.iter() {
        //    println!("G2Precomp coeff_i: {:?}", co_i);
        //}
        //println!("------ G2Precomp coeffs ------");
