...
impl Mul for Fq2 {
    ...
    fn mul(self, other: Fq2) -> Fq2 {
        ...
        let find = U256([41579711406032722041079298286863654543, 47913969641720662626022118191957660495]);
        if ((aa.raw()).0)[0] == (find.0)[0] && ((aa.raw()).0)[1] == (find.0)[1] {
            let aa_norm = U256::from(aa);
            //let mut aa_norm_be = [0u8; 32];
            println!("aa_norm: {:?}", aa_norm);
            /*
            ...
            */
            ...
            let mut aa_norm_2: U256 = aa.into();
            println!("aa_norm_2 from into: {:?}", aa_norm_2);
            ...
            println!("aa_norm_2 after conversion from mont to normal: {:?}", aa_norm_2);
            println!("is aa_norm_2 bigger than the modulus?  {:?}", aa_norm_2 > Fq::modulus());
            println!("Fq::modulus(): {:?}", Fq::modulus());
            //println!("Fq::modulus(): {:#04x}", Fq::modulus());
            println!("aa_norm_2 after conversion from mont to normal: {:?}", aa_norm_2);

            let mut aa_norm_2_be = [0u8; 32];
            aa_norm_2.to_big_endian(&mut aa_norm_2_be).unwrap();
            println!("aa_norm_2 in big endian: {:?}", aa_norm_2);

            let mut aa_norm_be = [0u8; 64];
            aa_norm.to_big_endian(&mut aa_norm_be[32..64]).unwrap();
            let aa_as_fq = Fq::interpret(&aa_norm_be);
            println!("calculated 1f47f60c7...  normal form to Fq: {:?}", aa_as_fq);
        }
        */
